...
<head>
	<meta charset="utf-8"> 
	<title>所有用户信息</title>
	<%-- 	<link rel="stylesheet" href="<%=request.getContextPath()%>/bootstrap/bs.min.css"> --%>
<%-- <script src="<%=request.getContextPath()%>/bootstrap/bs.min.js"></script> --%>
				...
				$.ajax({
			     	...
			     	success:function(rs){
			     		...
			     		for(var i in rs){
			     			...
			     			str += "<td>"+rs[i].user_name+"</td><td>"+rs[i].sex+"<td>"+rs[i].age+"</td><td>"+rs[i].email+"</td>";
...
// 			     			str += "<td>"+rs[i].update_time+"</td>";
			...
			detail: function(){
				console.log("detail");
				$("#msgBody>tr>th:gt(0)").on("click", function(){
					var id = $(this).find("td:eq(5)").text();
					window.location.href="<%=request.getContextPath()%>/jsp/admin/detail.jsp?id="+id;
				});
			},
			colorCh: function(){
				var oldColor= null;
