...
<!-- 引入必须的css和js文件 -->
  ...
  <nav class="navbar navbar-default" role="navigation">
    ...
    <div class="navbar-header">
        <a class="navbar-brand" href="#">同意</a>
    </div>
    </div>
</nav>
...
// 							alert(isConfirm);
		  ...
		  table:function(){
			 t =  $('#example').DataTable({
			      ajax: {
			          //指定数据源
			          url: "<%=request.getContextPath()%>/friendController/listRequire"
			      },
			      //每页显示三条数据
			      pageLength: 3,
			      "language": {
...
//			               return '<a href="' + data + '" target="_blank">' + row.title + '</a>';
			      ...
			      },
			      {
			          "render": function(data, type, row, meta) {
			              //渲染 把数据源中的标题和url组成超链接
//			               return '<a href="' + data + '" target="_blank">' + row.title + '</a>';
							if(data===1)return "<button type=\"button\" class=\"btn btn-info btn-add\">同意</button>"
							+
							"<button type=\"button\" class=\"btn btn-warning btn-deny\">拒绝</button>";
			          },
			          //指定是第三列
			          "targets": 4
			      }]
